...
<head>
    ...
    <title>District Collector - Update Problem Material</title>
        ...
        <div class="hk-pg-wrapper">
            ...
            <nav class="hk-breadcrumb" aria-label="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="DistrictCollectorDashboard.php">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="view_problem_material.php">Manage Problem Material</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Update Problem Material</li>
                </ol>
            </nav>
            <!-- /Breadcrumb -->
            ...
            <div class="container">
                ...
                <div class="hk-pg-header">
                    <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>Update Problem Material</h4>
                </div>
                <!-- /Title -->
                        ...
                        <section class="hk-sec-wrapper">
                            <div class="row">
                                ...
                                <?php 
                                    ...
                                    {
                                        $res= mysqli_query($con,"select * from tbl_problem_material where problem_material_id='". $_REQUEST["edit"]."'");
                                        while($row=mysqli_fetch_array($res))
                                        {
                                            $id=$row["problem_material_id"];                                            
                                            $problem_material_url=$row["problem_material_url"];
                                            
                                            $problem_material_type=$row["problem_material_type"];
                                 ...
                                 {
                                      $result= mysqli_query($con,"update tbl_problem_material set problem_st_id='".$_REQUEST["problem_st_id"]."' 
                                      where problem_material_id='".$_REQUEST["txthfpmid"]."'") or die(mysqli_error($con));
                                
                                      if($result)
                                      {
                                        $id=$_REQUEST["txthfpmid"];

                                        if($_FILES["problem_material_url"]["name"] !=NULL)
                                        {
                                            $source=$_FILES["problem_material_url"]["tmp_name"];
                                            $filename= $_FILES["problem_material_url"]["name"];
                                            $ext = pathinfo($filename, PATHINFO_EXTENSION);
                                            $newname="PM_".date('Ymdhis').".".$ext;

                                             move_uploaded_file($source,"../uploads/problem_material_url/".$newname);

                                             mysqli_query($con,"update tbl_problem_material set problem_material_url ='".$newname."' where problem_material_id='".$id."'") or die(mysqli_error($con));
                                       

                                          echo "<script>window.location='view_problem_material.php';</script>";
                                        }
                                      } else {
                                ...
                                <input type="hidden" value="<?php echo $id; ?>" name="txthfpmid" />  
                                        <div class="form-row">
                                        <div class="col-md-12 mb-10">
                                                <label for="validationCustom02">Problem</label>
                                                <select class="form-control custom-select" name="problem_st_id" required>
                                            ...
                                            <div class="form-row">
                                            <div class="col-md-12 mb-10">
                                                <label for="validationCustom03">Select Material</label>
                                                <div class="custom-file">
                                                    <input name="problem_material_url" type="file" value="<?php echo $problem_material_url; ?>"
                                                        class="custom-file-input" id="validatedCustomFile" required>
                                                    <label class="custom-file-label" for="validatedCustomFile"><?php echo $problem_material_url; ?></label>
                                                </div>
                                            </div>
                                            </div><!-- Form Row End -->
